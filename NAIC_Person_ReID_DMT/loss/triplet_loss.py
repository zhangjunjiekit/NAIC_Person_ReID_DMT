...
def euclidean_dist(x, y):
    ...
    dist = xx + yy
    dist.addmm_(beta=1, alpha=-2, mat1=x, mat2=y.t()) #out = beta* mat + alpha* (mat1 x  mat2)
    dist = dist.clamp(min=1e-12).sqrt()  # for numerical stability
    return dist

# 挖掘最难的正样本：对每个实例，找到在同一个batch内与其同类别的实例中两者特征最远的欧式距离
# 挖掘最难的负样本：对每个实例，找到在同一个batch内与其不同类别的实例中两者特征最近的欧式距离
def hard_example_mining(dist_mat, labels, return_inds=False):
    ...
    # print(dist_mat)
    # print(dist_mat.shape,is_pos.shape)
    # `dist_ap` means distance(anchor, positive)
    # both `dist_ap` and `relative_p_inds` with shape [N, 1]
    # print(dist_mat[is_pos])
    dist_ap, relative_p_inds = torch.max(
        dist_mat[is_pos].contiguous().view(N, -1), 1, keepdim=True)#在view中调用了contiguous方法，则可能在返回Tensor底层数据中使用了新的内存
    ...
    if return_inds:
        # shape [N, N]
        ind = (labels.new().resize_as_(labels)#new()用于创建一个type和device与labels一致的Tensor,但是没有内容。
...
# 通过困难样本的数据挖掘找出每个实例与对应的最困难样本之间的欧式距离去计算ranking_loss
...
# 直接计算每个实例与左右正样本和负样本之间欧式距离去计算ranking_loss
