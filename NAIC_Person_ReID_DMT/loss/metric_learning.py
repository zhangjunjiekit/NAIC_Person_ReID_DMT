...
# 直接在角度空间（angular space）中最大化分类界限，
class Arcface(nn.Module):
    ...
    def forward(self, input, label):
        ...
        phi = cosine * self.cos_m - sine * self.sin_m #三角公式 cos(a+b) = cos(a)*cos(b)-sin(a)*sin(b)
        ...
        # scatter_(input, dim, index, src)将src中数据根据index中的索引按照dim的方向填进input中,dim=1表示列（横向）方向
...
# https://blog.csdn.net/weixin_42111770/article/details/80690657
# 在余弦空间中最大化分类界限
class AMSoftmax(nn.Module):
    ...
    def forward(self, x, lb):
        ...
        costh = torch.mm(x_norm, w_norm)#矩阵相乘
