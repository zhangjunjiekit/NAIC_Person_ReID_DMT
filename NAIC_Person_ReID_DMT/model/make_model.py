...
# GeM既可以包含可学习的参数p，对输入样本先求p次幂，然后取均值，在进行p次开方。
# 通过指定固定的p,通过调节参数p，可以关注不同细度的区域
# GeM目前已经成为了图像检索池化操作的主流使用方法
...
class GeM(nn.Module):
    ...
    def forward(self, x):
        # adaptive_avg_pool2d 与 普通的池化层区别在于不需要给定核大小和步长，会根据输入输出自适应计算出。
        # x.clamp跟np.clip作用一样，用于截断区间外的数值
        # (1, 1) 为指定的输出大小
